...
#02. class_name
#-----------------------------------------------------------
class_name FoltaUICommandVList
...
@export var all_item_left_control_path:NodePath

@export var all_item_right_control_path:NodePath

@onready var all_item_left_control:Control = get_node_or_null(all_item_left_control_path)

@onready var all_item_right_control:Control = get_node_or_null(all_item_right_control_path)
...
func _on_safe_pressed_command(button):
	safe_pressed.emit(button.id, button)
	is_lock = true
	for command in command_list:
		if command != button:
			command.set_focus_exit(false,false)
...
func _set_all_item_neighbour_items()->void:
	_is_left_move_enabled = false
	_is_right_move_enabled = false
	...
	for child in children:
		if child is FoltaUIButton or child is FoltaUIButton:
			...
			if index != 0:
				...
				if child.focus_neighbor_top_default == NodePath(""):
					child.focus_neighbor_top = pathh

				if prev_command.focus_neighbor_bottom_default == NodePath(""):
					prev_command.focus_neighbor_bottom = prev_command.get_path_to(child)

				if all_item_left_control != null:
					var relative_left_path = child.get_path_to(all_item_left_control)
					child.focus_neighbor_left = relative_left_path
					_is_left_move_enabled = true

				if all_item_right_control != null:
					var relative_right_path = child.get_path_to(all_item_right_control)
					child.focus_neighbor_right = relative_right_path
					_is_right_move_enabled = true
	...
	if command_list[0].focus_neighbor_top_default == NodePath(""):
		command_list[0].focus_neighbor_top = command_list[0].get_path_to(command_list.back())
	if command_list.back().focus_neighbor_bottom_default == NodePath(""):
		command_list.back().focus_neighbor_bottom = command_list.back().get_path_to(command_list[0])
...
func _move_focus_command():
	...
	if !_is_left_move_enabled and is_left:
		return
	if !_is_right_move_enabled and is_right:
		return
