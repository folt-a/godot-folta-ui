...
#02. class_name
#-----------------------------------------------------------
class_name FoltaUICommandGridList
...
func _set_all_item_neighbour_items()->void:
	...
	for child in children:
		if child is FoltaUIButton or child is FoltaUIButton:
			...
			if command_row_list.size() <= row_index:
				command_row_list.append([])
			command_row_list[row_index].append(child)
			child.command_list = self

			col_index += 1
			index += 1
	
	# TODO 右端→次の段の左端にしたいかも
	...
	for command_row in command_row_list:
		...
		for command in command_row:
			...
			if row_index == 0:
				# 一番上から一番下に移動
				if command.focus_neighbor_top_default == NodePath(""):
					if command_row_list.back().size() - 1 >= col_index:
						command.focus_neighbor_top = command.get_path_to(command_row_list.back()[col_index])
					else:
						# 行がきっちり終わっていない場合（3列のグリッドに最後が2列で終わるなど）は、
						# 最後から2番目の行の同位置にする
						command.focus_neighbor_top = command.get_path_to(command_row_list[-2][col_index])
			else:
				if command.focus_neighbor_top_default == NodePath(""):
					command.focus_neighbor_top = command.get_path_to(command_row_list[row_index - 1][col_index])
			# LEFT
			if col_index == 0:
				# 最左
				if command.focus_neighbor_left_default == NodePath(""):
					command.focus_neighbor_left = command.get_path_to(command_row.back())
			else:
				if command.focus_neighbor_left_default == NodePath(""):
					command.focus_neighbor_left = command.get_path_to(command_row_list[row_index][col_index - 1])
			# RIGHT
			if col_index == command_row.size() - 1:
				# 最右
				if command.focus_neighbor_right_default == NodePath(""):
					command.focus_neighbor_right = command.get_path_to(command_row.front())
			else:
				if command.focus_neighbor_right_default == NodePath(""):
					command.focus_neighbor_right = command.get_path_to(command_row_list[row_index][col_index + 1])
			# BOTTOM
			if row_index == command_row_list.size() - 1:
				# 一番↓から一番↑に移動
				if command.focus_neighbor_bottom_default == NodePath(""):
					command.focus_neighbor_bottom = command.get_path_to(command_row_list.front()[col_index])
			else:
				if command_row_list[row_index + 1].size() - 1 >= col_index:
					command.focus_neighbor_bottom = command.get_path_to(command_row_list[row_index + 1][col_index])
				else:
					# 行がきっちり終わっていない場合（3列のグリッドに最後が2列で終わるなど）は、
					# 一番↓から一番↑に移動する
					command.focus_neighbor_bottom = command.get_path_to(command_row_list.front()[col_index])
